...
class TemporaryMeaningFragment(private val doorId: Int) : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_temporary_meaning, container, false)
        ...
        ivPanda.setOnTouchListener { v, event ->
            if (event.action == MotionEvent.ACTION_UP) {
                ...
                if (event.x in headLeft..headRight && event.y in headTop..headBottom) {
                    currentTrial++
                    if (currentTrial >= AppConfig.TemporaryMeaning.TRIAL_COUNT) {
    ...
    private fun startTrial() {
        ...
        tvPrompt.text = AppConfig.TemporaryMeaning.TRIAL_TEXTS[currentTrial]

        val images = AppConfig.TemporaryMeaning.TRIAL_IMAGES[currentTrial]
