...
class DiscourseNoveltyFragment(private val doorId: Int) : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_discourse_novelty, container, false)
        ...
        ivPanda.setOnTouchListener { v, event ->
            if (event.action == MotionEvent.ACTION_UP) {
                ...
                if (event.x in headLeft..headRight && event.y in headTop..headBottom) {
                    currentTrial++
                    if (currentTrial >= AppConfig.DiscourseNovelty.TRIAL_COUNT) {
    ...
    private fun startTrial() {
        ...
        tvPrompt.text = AppConfig.DiscourseNovelty.TRIAL_TEXTS[currentTrial]

        val images = AppConfig.DiscourseNovelty.TRIAL_IMAGES[currentTrial]
